...
import { UrlAdminLabs, UrlAdminDeleteLab, UrlAdminBulkDeleteLabs } from 'ajax/apiUrls';
import { useNavigate } from "react-router-dom";
import { showEditLabModal } from './EditLabModal/ShowEditLabModal';

const AdminLabPage = ({ history }: any) => {
  ...
  const editRow = async (record: any) => {
    let dlgRes = await showEditLabModal({
      data: record,
      method: "edit",
      show: true,
      onSuccess: fetchData,
  ...
  const deleteRow = async (key: any) => {
    ...
    apiDelete(`${UrlAdminDeleteLab}/${key}`)
      .then(() => {
        Toast('Lab deleted successfully!', "success");
      })
      .catch((err) => {
        Toast(err, "error");
      });
